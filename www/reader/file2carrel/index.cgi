...
# file2queue.cgi - given a few inputs, add a carrel configuration to the queue

# Eric Lease Morgan <emorgan@nd.edu>
# (c) University of Notre Dame; distributed under a GNPU Public License

# November 30, 2020 - my very own Arivata; on Thanksgiving vacation in Lancaster during a pandemic
...
use constant TYPE    => 'file2carrel';
use constant POSTMAX => 1024 * 20000;
...
# do the work
	...
	# get the basename and extension of the loaded file, and move it to the backlog
	my ( $foo, $bar, $extension ) = fileparse( $handle, qr/\.[^.]*/ );
	my $file = $cgi->tmpFileName( $handle );
	my ( $name, $foo, $bar ) = fileparse( $file, qr/\.[^.]*/ );
	move( $file, BACKLOG . "/$name$extension" ) or die "Can't move $file ($!). Call Eric.";
	...
	print QUEUE join( "\t", ( TYPE, $shortname, $date, $time, $username, "$name$extension" ) ), "\n";
	close QUEUE;
